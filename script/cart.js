...
function displayCart(cart) {
  ...
  cart.forEach((product) => {
    ...
    div.innerHTML = `<img src="${product.image}" width="100" height="100">
          <h3>${product.name}</h3>
  ...
  avgPriceElement.textContent = totalItems ? (total / totalItems).toFixed(2) : 0;
...
function filterCart() {
  let cart = JSON.parse(localStorage.getItem("cart")) || [];
  const filterPrice =
    parseFloat(document.getElementById("filterPrice").value) || 0;

  console.log("Filter Price:", filterPrice);
  console.log("Cart Before Filtering:", cart);

  if (!isNaN(filterPrice)) {
    cart = cart.filter((product) => Number(product.price) > filterPrice);
  }
